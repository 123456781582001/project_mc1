...
int main()
{
    ...
    Dio_enuWriteChannel(DIO_CHANNEL09,DIO_u8LOW); //pin of led
	u8 Send_data=0,data;
	u8 button=0;
	while(1)
	{
		Dio_enuReadChannel(DIO_CHANNEL27,&button);// button connect in pin value of button
		while(button==1)
		{
		...
		while((Send_data>0) && (Send_data<=9))
		{
			...
			Uart_SendData(data);//function send data
			Send_data=0;
		}
    }
	return 0;
